...
if(n>0){
  ...
  if(n>1){
    ...
    if(n>2){
      H3 <- as.character( fread(list.files()[grep("MCS3", list.files())], nrows = 1, header = FALSE))
      MCS3 <- fread(list.files()[grep("MCS3", list.files())], skip = 2, col.names = H3[-length(H3)])
      if(n>3){
        H4 <-as.character( fread(list.files()[grep("MCS4", list.files())], nrows = 1, header = FALSE))
        MCS4 <- fread(list.files()[grep("MCS4", list.files())], skip = 2, col.names = H4[-length(H4)])
      }
    }
  }
}
...
#-------------- COMBINE MCS ------------------------------------------------------------------------
if(n>0){#format each MCS tables ( take out duplicate columns)
  MCS1 <- MCS1[ , !which(duplicated(colnames(MCS1))), with = FALSE]
  MCS <- MCS1
  if(n>1){
    MCS2 <- MCS2[ , !which(duplicated(colnames(MCS2))), with = FALSE]
    MCS <- merge(MCS, MCS2, by = "Time")
    if(n>2){
      MCS3 <- MCS3[ , !which(duplicated(colnames(MCS3))), with = FALSE]
      MCS <- merge(MCS, MCS3, by = "Time")
      if(n>3){
        MCS4 <- MCS4[ , !which(duplicated(colnames(MCS4))), with = FALSE]
        MCS <- merge(MCS, MCS4, by = "Time")
      }
    }
  }
...
#create the final combined MCS
MCSF <- data.frame()


#Separate into separate gobs. 
#LOGIC if detected pattern "GA" reads false for all columns of MCS table
#there are no GA gobs present. 
if(sum(str_detect(names(MCS), "GA-")==FALSE)!=ncol(MCS)){
...
if(sum(str_detect(names(MCS), "GB-")==FALSE)!=ncol(MCS)){
...
if(sum(str_detect(names(MCS), "GC-")==FALSE)!=ncol(MCS)){
  C <- select(MCS, 'Time', 'F-BTM', names(MCS)[grep("GC", names(MCS))])
  C <- C %>% mutate(Gob = rep("C", nrow(C)))
  names(C) <- gsub("GC-", "",names(C))
  MCSF <- rbind(MCSF, C)
}

if(sum(str_detect(names(MCS), "GD-")==FALSE)!=ncol(MCS)){
  D <- select(MCS, 'Time', 'F-BTM', names(MCS)[grep("GD", names(MCS))])
  D <- D %>% mutate(Gob = rep("D", nrow(D)))
  names(D) <- gsub("GD-", "",names(D))
  MCSF <- rbind(MCSF, D)
}
...
#"Konatic003-70-004" == FP2
if(unique(DM$`Reader Number`) == "Konatic003-70-004"){
  #filter for FP2
  FP <- filter(FP, FP == 2)} 
#"Konatic003-70-002" == FP2
if(unique(DM$`Reader Number`) == "Konatic003-70-002"){
  #filter for FP3
  FP <- filter(FP, FP == 3)} 
...
if(dif < 0){
  FP <- FP[1:nrow(DM),]
  message(paste(abs(dif), "rows deleted from FP"))
}
if(dif == 0){
  message("Datamatrix and Inspection reader have same number of rows")
}

#have a unique Time column
temp <- cbind(DM, FP[,-1])

#Combine hot and cold end by gob

names(MCSF) <- sub('Time', 'CreateTime', names(MCSF))
merged <- join(MCSF, temp, by = c('CreateTime', 'Gob'), type = 'right', match = 'all')
head(merged) 
